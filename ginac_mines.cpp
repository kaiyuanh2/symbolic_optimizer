...
int main() {
    ofstream outfile("ginac_output_mines.txt");
    if (!outfile.is_open()) {
    ...
    // Load validation set
    matrix X_test = load_matrix_from_csv("X_val_mines.csv");
    outfile << "X_test: " << X_test.rows() << "x" << X_test.cols() << endl;
    
    matrix y_test = load_matrix_from_csv("y_val_mines.csv");
    outfile << "y_test: " << y_test.rows() << "x" << y_test.cols() << endl;
    
    // Load parameters
    ifstream param_file("param_ginac_mines_lr.txt");
    vector<ex> param_vec;
    string line;
    ...
    try {
        ...
        ofstream result_file("ginac_result_mines.txt");
