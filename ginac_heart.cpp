...
int main() {
    ...
    ofstream outfile("ginac_output_heart.txt");
    if (!outfile.is_open()) {
    ...
    // Load validation set
    matrix X_test = load_matrix_from_csv("X_val_heart.csv");
    outfile << "X_test: " << X_test.rows() << "x" << X_test.cols() << endl;
    
    matrix y_test = load_matrix_from_csv("y_val_heart.csv");
    outfile << "y_test: " << y_test.rows() << "x" << y_test.cols() << endl;
    
    // Load parameters
    ifstream param_file("param_ginac_heart_lr.txt");
    vector<ex> param_vec;
    string line;
    ...
    try {
        ...
        ofstream result_file("ginac_result_heart.txt");
